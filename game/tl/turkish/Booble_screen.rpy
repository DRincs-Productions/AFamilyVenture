# TODO: Translation updated at 2022-12-01 22:29

# game/Booble_screen.rpy:51
translate turkish booble_search_f6ce98ce:

    # RT "{i}I should do a search on booble.com to find a way to buy the stuff I need so that the FBI won't ask me from where I got the money.{/i}"
    RT "{i}İhtiyacım olan şeyleri satın almanın bir yolunu bulmak için booble.com'da arama yapmalıyım, böylece FBI bana parayı nereden bulduğumu sormaz.{/i}"

# game/Booble_screen.rpy:67
translate turkish booble_search_ce876757:

    # RT "{i}What should I do today on the internet?{/i}"
    RT "{i}Bugün internette ne yapmalıyım?{/i}"

translate turkish strings:

    # game/Booble_screen.rpy:33
    old "{color=#ffffff}{b}hardnLong.dong online shopping{/b}{/color}"
    new "{color=#ffffff}{b}hardnLong.dong online alışveriş{/b}{/color}"

    # game/Booble_screen.rpy:39
    old "{color=#ffffff}{b}cosplayheaven.cos cosplay site{/b}{/color}"
    new "{color=#ffffff}{b}cosplayheaven.cos cosplay sitesi{/b}{/color}"

